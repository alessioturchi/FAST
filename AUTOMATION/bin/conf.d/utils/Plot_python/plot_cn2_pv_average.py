...
class Datastore(object):
    def __init__(self,files) :
        ...
        self.cn2=data_all[:,1,:]
        self.cn2_sd=data_all[:,2,:]
        self.cn2low=self.cn2-self.cn2_sd
        self.cn2low[self.cn2low<10e-21]=10e-21
        self.cn2up=self.cn2+self.cn2_sd
...
def main():
    ...
    for i in range(nfiles):
        ...
        stringaOS18=''
        if (os.path.basename(files[i]).split('.')[2] == 'OS18'):
            stringaOS18='_OS18'
        ...
        #Plot cn2
        title_label='$C_N^2$ - Averaged in ['+hd.stringaora+']h UT or ['+hd.stringaoralt+']h MST'
        xpoly=np.concatenate((data.cn2up[:,i],data.cn2low[::-1,i]))
        ...
        plt.plot(data.cn2[:,i],data.heights,color='k',linewidth=1)
        plt.plot(data.cn2low[:,i],data.heights,':',color='k',linewidth=0.5)
        plt.plot(data.cn2up[:,i],data.heights,':',color='k',linewidth=0.5)
        plt.xscale('log')
...
#        plt.axes().set_aspect(0.36)
        ...
        ax = plt.gca()
        plt.text(0, -0.16,stringadata_fig,verticalalignment='center',transform = ax.transAxes, fontsize=font_size+1)
        plt.xlim(minval,maxval)
        plt.ylim(hd.heightlow,hd.heighthigh)
        plt.tight_layout()
#        plt.subplots_adjust(left=0.09, right=0.95, top=0.93, bottom=0.13)
        
        fileout="DATE_cn2_pv_avg_0"+str(i+1)+stringaOS18+".png"
        plt.savefig(fileout,dpi=my_dpi)
        print("Creato file con plot:", fileout)

        #Write out dat file
        fname="DATE_cn2_pv_avg_0"+str(i+1)+stringaOS18+".dat"
        f = open(fname, 'w')
        f.write('### CN2 VERTICAL PROFILE TIME AVERAGED IN '+hd.stringaora+' UT '+'\n')
        f.write('### HEIGHT(m) CN2_AVG(m^-2/3) SIGMA_CN2(m^-2/3)'+'\n')
        printdata=np.zeros((1,3),float)
        for j in range(data.rows):
            printdata[0,0]=data.heights[j]
            printdata[0,1]=data.cn2[j,i]
            printdata[0,2]=data.cn2_sd[j,i]
            np.savetxt(f,printdata,fmt='%E',delimiter='\t')
        f.close()

#FINAL PLOT WITH SUPERIMPOSED FIGURES
    #Plot cn2 speed
    title_label='$C_N^2$ - Averaged vertical profiles'
    plt.figure(figsize=(1676./my_dpi, 1303./my_dpi), dpi=my_dpi)
    plt.rcParams['axes.labelsize'] = font_size
    plt.rcParams['xtick.labelsize'] = font_size
    plt.rcParams['ytick.labelsize'] = font_size
    plt.plot(data.cn2[:,0],data.heights,color='r',linewidth=1,label=stringheora_ut[0]+' UT - '+stringheora_ut[0]+' MST')
    plt.plot(data.cn2[:,1],data.heights,color='b',linewidth=1,label=stringheora_ut[1]+' UT - '+stringheora_ut[1]+' MST')
    plt.plot(data.cn2[:,2],data.heights,color='g',linewidth=1,label=stringheora_ut[2]+' UT - '+stringheora_ut[2]+' MST')
    plt.xscale('log')    
...
#        plt.axes().set_aspect(0.36)
    ...
    plt.ylabel('Height (km) a.g.l.')
    ax = plt.gca()
    plt.text(0, -0.15,stringadata_fig,verticalalignment='center',transform = ax.transAxes, fontsize=font_size+1)
    plt.xlim(minval,maxval)
    plt.ylim(hd.heightlow,hd.heighthigh)
    plt.tight_layout()
#    plt.subplots_adjust(left=0.09, right=0.95, top=0.77, bottom=0.1)
    fileout="DATE_cn2_pv_avg_ALL"+stringaOS18+".png"
    plt.savefig(fileout,dpi=my_dpi)
    print("Creato file con plot:", fileout)
