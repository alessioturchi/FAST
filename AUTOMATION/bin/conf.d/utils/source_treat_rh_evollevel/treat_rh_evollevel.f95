...
!!Perform resampling average
  ...
  if ( starttime >= npoints) then
    print*, '!!! treat_rh_evollevel: start time is >= than total time'
    call exit(1)
  end if
  if ( endtime > npoints) then
    print*, '!!! treat_rh_evollevel: end time is greater than total time'
    call exit(1)
  end if
...
!Write out the file to pass to the plotting routine
  ...
  write(20,*) '### timestep (s), start time (s), endtime (s), RH_MEAN, RH_SD ###'
  write(20,*) 1200,time(starttime),time(endtime), temp_mean, temp_sd
  write(20,*) '### TIME (s), RELATIVE HUMIDITY, RH_STANDARD_DEVIATION ###'
  do i=1,dim_array_res
  ...
  write(20,*) '### timestep (s), start time (s), endtime (s), RH_MEAN, RH_SD ###'
  write(20,*) 1200,time(starttime),time(endtime), temp_mean, temp_sd
  write(20,*) '### TIME (s), RELATIVE HUMIDITY ###'
  do i=1,npoints
    write(20,*) time(i),temperature(i)
  end do
  close(20)

end program treat_rh_evollevel
