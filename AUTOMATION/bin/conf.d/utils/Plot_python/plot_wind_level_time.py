...
def main():
    ...
    hd=Header(files[0],UTOFFSET+HOUR_SHIFT_PLOT)
    ...
    for i in range(nfiles):
        ...
        #Load header
        hd=Header(files[i],UTOFFSET+HOUR_SHIFT_PLOT)
        #Set limits
        ...
        #Plot wind speed
        title_label='Wind speed - Temporal evolution on level k='+level.strip()
        ...
        plt.errorbar(data.time,data.wind[:,i], yerr=data.wind_sd[:,i],color='k',linewidth=2, ecolor='#878b91',elinewidth=0.5,capsize=2)
...
#        plt.subplots_adjust(left=0.12, right=0.98, top=0.84, bottom=0.11) 
        ...
        fileout="DATE_wind_k"+level.strip()+"_evol_time.png"
        plt.savefig(fileout,dpi=my_dpi)
        print("Creato file con plot:", fileout)

        #Write out dat file
        fname="DATE_wind_k"+level.strip()+"_evol_time.dat"
        f = open(fname, 'w')
        f.write('### WIND SPEED LEVEL K='+level.strip()+' TIME EVOLUTION'+'\n')
        f.write('### TIME(h) WIND(m/s) SIGMA_WIND(m/s)'+'\n')
        printdata=np.zeros((1,3),float)
        for j in range(len(printtime)):
            printdata[0,0]=printtime[j]
            printdata[0,1]=printwind[j,i]
            printdata[0,2]=printwind_sd[j,i]
