...
class Datastore(object):
    def __init__(self,files) :
        ...
        #Get data in g/Kg
        self.wapor=data_all[:,1,:]*1000.
        self.wapor_sd=data_all[:,2,:]*1000.
        self.waporlow=self.wapor-self.wapor_sd
        self.waporup=self.wapor+self.wapor_sd
...
def main():
    ...
    #Set limits
    maxval=np.max(data.wapor+data.wapor_sd)
    minval=np.min(data.wapor-data.wapor_sd)
    ...
    for i in range(nfiles):
        ...
        #Plot wapor
        title_label='Mixing Ratio - Averaged in ['+hd.stringaora+']h UT or ['+hd.stringaoralt+']h MST'
        xpoly=np.concatenate((data.waporup[:,i],data.waporlow[::-1,i]))
        ...
        plt.plot(data.wapor[:,i],data.heights,color='k',linewidth=1)
        plt.plot(data.waporlow[:,i],data.heights,':',color='k',linewidth=0.5)
        plt.plot(data.waporup[:,i],data.heights,':',color='k',linewidth=0.5)
#        plt.xscale('log')
...
#        plt.subplots_adjust(left=0.09, right=0.95, top=0.93, bottom=0.15)
        ...
        fileout="DATE_wapor_pv_avg_0"+str(i+1)+".png"
        plt.savefig(fileout,dpi=my_dpi)
        print("Creato file con plot:", fileout)

        #Write out dat file
        fname="DATE_wapor_pv_avg_0"+str(i+1)+".dat"
        f = open(fname, 'w')
        f.write('### MIXING RATIO VERTICAL PROFILE TIME AVERAGED IN '+hd.stringaora+' UT '+'\n')
        f.write('### HEIGHT(m) MR_AVG(g/kg) SIGMA_MR(g/kg)'+'\n')
        printdata=np.zeros((1,3),float)
        for j in range(data.rows):
            printdata[0,0]=data.heights[j]
            printdata[0,1]=data.wapor[j,i]
            printdata[0,2]=data.wapor_sd[j,i]
            np.savetxt(f,printdata,fmt='%E',delimiter='\t')
        f.close()

#FINAL PLOT WITH SUPERIMPOSED FIGURES
    #Plot wapor 
    title_label='Mixing Ratio - Averaged vertical profiles'
    plt.figure(figsize=(1676./my_dpi, 1303./my_dpi), dpi=my_dpi)
    plt.rcParams['axes.labelsize'] = font_size
    plt.rcParams['xtick.labelsize'] = font_size
    plt.rcParams['ytick.labelsize'] = font_size
    plt.plot(data.wapor[:,0],data.heights,color='r',linewidth=1,label=stringheora_ut[0]+' UT - '+stringheora_ut[0]+' MST')
    plt.plot(data.wapor[:,1],data.heights,color='b',linewidth=1,label=stringheora_ut[1]+' UT - '+stringheora_ut[1]+' MST')
    plt.plot(data.wapor[:,2],data.heights,color='g',linewidth=1,label=stringheora_ut[2]+' UT - '+stringheora_ut[2]+' MST')
...
#    plt.subplots_adjust(left=0.09, right=0.95, top=0.77, bottom=0.12)
    fileout="DATE_wapor_pv_avg_ALL.png"
    plt.savefig(fileout,dpi=my_dpi)
    print("Creato file con plot:", fileout)
...
if __name__ == '__main__':
    try:
        ...
        print("!!!Error: error plotting plot_wapor_pv_avg.py with input parameters: ",sys.argv)
