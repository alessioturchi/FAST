...
class Datastore(object):
    def __init__(self,files) :
        ...
        self.wapor=data_all[:,1]
        self.wapor_sd=data_all[:,2]
...
def main():
    ...
    #APPLY STATISTICAL CORRECTION TO THE WATER VAPOR
#    data.wapor=(data.wapor/1.04)-0.25
#    print("*Python********************")
#    print("***************************")
#    print("PWV CORRECTION: data.wapor=(data.wapor/1.04) -0.25")
#    print("***************************")
#    print("***************************")
    ...
    HOUR_SHIFT_PLOT=int(sys.argv[10])
    ...
    printwapor=data.wapor[start_index:end_index+1]
    printwapor_sd=data.wapor_sd[start_index:end_index+1]
    ...
    hd=Header(files,UTOFFSET)
    #Set limits
    maxval=np.max(data.wapor[start_index:end_index]+data.wapor_sd[start_index:end_index])
    minval=np.min(data.wapor[start_index:end_index]-data.wapor_sd[start_index:end_index])
    ...
    if (hd.startheight < 200):
        ...
        stringahtitle='integrated in ['+str(hd.endheight).strip()+'-'+str(hd.endheight).strip()+']m'

    #Plot wapor
    title_label='PWV - Temporal evolution - '+stringahtitle.strip()
    ...
    plt.errorbar(data.time,data.wapor[:], yerr=data.wapor_sd[:],color='k',linewidth=2, ecolor='#878b91',elinewidth=0.5,capsize=2)
...
#    plt.axes().set_aspect(0.36)
    ...
    plt.ylabel('PWV (mm)',fontsize=font_size+2)
    ax = plt.gca()
...
#    plt.subplots_adjust(left=0.11, right=0.98, top=0.84, bottom=0.11) 
    ...
    fileout="DATE_wapor_evol_time_Height_"+stringah+'.png'
    plt.savefig(fileout,dpi=my_dpi)
    print("Creato file con plot:", fileout)

    #Write out dat file
    fname="DATE_wapor_evol_time_Height_"+stringah+'.dat'
    f = open(fname, 'w')
    f.write('### PWV TIME EVOLUTION INTEGRATED IN '+stringah+' meters'+'\n')
    f.write('### TIME(h) PWV(mm) SIGMA_PWV(mm)'+'\n')
    printdata=np.zeros((1,3),float)
    for j in range(len(printtime)):
        printdata[0,0]=printtime[j]
        printdata[0,1]=printwapor[j]
        printdata[0,2]=printwapor_sd[j]
...
if __name__ == '__main__':
    try:
        ...
        print("!!!Error: error plotting plot_wapor_time.py with input parameters: ",sys.argv)
