...
class Datastore(object):
    def __init__(self,files) :
        ...
        self.windlow=self.wind-self.wind_sd
        self.windup=self.wind+self.wind_sd
        ...
        for i in range(nfiles):
            ...
            self.maxarraywd[i]=np.max(self.wd[:,i])
            self.minarraywd[i]=np.min(self.wd[:,i])
        self.wdlow=self.wd-self.wd_sd
        self.wdup=self.wd+self.wd_sd
...
def main():
    ...
    #Set limits
    maxval=np.max(data.wind+data.wind_sd)
    minval=np.min(data.wind-data.wind_sd)
    maxval_wd=np.max(data.wd+data.wd_sd)
    minval_wd=np.min(data.wd-data.wd_sd)
    ...
    for i in range(nfiles):
        ...
        #Plot wind speed
        title_label='Wind speed - Averaged in ['+hd.stringaora+']h UT or ['+hd.stringaoralt+']h MST'
        xpoly=np.concatenate((data.windup[:,i],data.windlow[::-1,i]))
        ...
        plt.plot(data.wind[:,i],data.heights,color='k',linewidth=1)
        plt.plot(data.windlow[:,i],data.heights,':',color='k',linewidth=0.5)
        plt.plot(data.windup[:,i],data.heights,':',color='k',linewidth=0.5)
...
#        plt.subplots_adjust(left=0.09, right=0.95, top=0.93, bottom=0.15)
        ...
        fileout="DATE_wind_pv_avg_0"+str(i+1)+".png"
        plt.savefig(fileout,dpi=my_dpi)
        print("Creato file con plot:", fileout)

        #Write out dat file
        fname="DATE_wind_pv_avg_0"+str(i+1)+".dat"
        f = open(fname, 'w')
        f.write('### WIND SPEED VERTICAL PROFILE TIME AVERAGED IN '+hd.stringaora+' UT '+'\n')
        f.write('### HEIGHT(m) WIND_AVG(m/s) SIGMA_WIND(m/s)'+'\n')
        printdata=np.zeros((1,3),float)
        for j in range(data.rows):
            printdata[0,0]=data.heights[j]
            printdata[0,1]=data.wind[j,i]
            printdata[0,2]=data.wind_sd[j,i]
            np.savetxt(f,printdata,fmt='%E',delimiter='\t')
        f.close()

        #Plot wind direction
        title_label='Wind direction - Averaged in ['+hd.stringaora+']h UT or ['+hd.stringaoralt+']h MST'
        xpoly=np.concatenate((data.wdup[:,i],data.wdlow[::-1,i]))
        ypoly=np.concatenate((data.heights,data.heights[::-1]))
        plt.figure(figsize=(1676./my_dpi, 1003./my_dpi), dpi=my_dpi) 
        plt.rcParams['axes.labelsize'] = font_size
        plt.rcParams['xtick.labelsize'] = font_size
        plt.rcParams['ytick.labelsize'] = font_size
        plt.plot(data.wd[:,i],data.heights,color='k',linewidth=1)
        plt.plot(data.wdlow[:,i],data.heights,':',color='k',linewidth=0.5)
        plt.plot(data.wdup[:,i],data.heights,':',color='k',linewidth=0.5)
        plt.fill(xpoly,ypoly,color='#87CEEB')
#        plt.axes().set_aspect(0.36)
        plt.xticks(angle_values,angle_names)
        plt.tick_params(which='both')
        plt.tick_params(which='major', length=7)
        plt.tick_params(which='minor', length=4)
        plt.minorticks_on()
        ...
        ax = plt.gca()
        plt.text(0, -0.13,stringadata_fig,verticalalignment='center',transform = ax.transAxes, fontsize=font_size+1)
        plt.xlim(margin_left,margin_right)        
        plt.ylim(hd.heightlow,hd.heighthigh)
        plt.tight_layout()
#        plt.subplots_adjust(left=0.09, right=0.95, top=0.93, bottom=0.15)
        
        fileout="DATE_winddir_pv_avg_0"+str(i+1)+".png"
        plt.savefig(fileout,dpi=my_dpi)
        print("Creato file con plot:", fileout)

        #Write out dat file
        fname="DATE_winddir_pv_avg_0"+str(i+1)+".dat"
        f = open(fname, 'w')
        f.write('### WIND DIRECTION VERTICAL PROFILE TIME AVERAGED IN '+hd.stringaora+' UT '+'\n')
        f.write('### HEIGHT(m) WD_AVG(deg) SIGMA_WD(deg)'+'\n')
        printdata=np.zeros((1,3),float)
        for j in range(data.rows):
            printdata[0,0]=data.heights[j]
            printdata[0,1]=data.wd[j,i]
            printdata[0,2]=data.wd_sd[j,i]
            np.savetxt(f,printdata,fmt='%E',delimiter='\t')
        f.close()
...
#FINAL PLOT WITH SUPERIMPOSED FIGURES
    #Plot wind speed
    title_label='Wind speed - Averaged vertical profiles'
    plt.figure(figsize=(1676./my_dpi, 1303./my_dpi), dpi=my_dpi)
    plt.rcParams['axes.labelsize'] = font_size
    plt.rcParams['xtick.labelsize'] = font_size
    plt.rcParams['ytick.labelsize'] = font_size
    plt.plot(data.wind[:,0],data.heights,color='r',linewidth=1,label=stringheora_ut[0]+' UT - '+stringheora_ut[0]+' MST')
    plt.plot(data.wind[:,1],data.heights,color='b',linewidth=1,label=stringheora_ut[1]+' UT - '+stringheora_ut[1]+' MST')
    plt.plot(data.wind[:,2],data.heights,color='g',linewidth=1,label=stringheora_ut[2]+' UT - '+stringheora_ut[2]+' MST')
...
#    plt.subplots_adjust(left=0.09, right=0.95, top=0.77, bottom=0.12)
    fileout="DATE_wind_pv_avg_ALL.png"
    plt.savefig(fileout,dpi=my_dpi)
    print("Creato file con plot:", fileout)

    #Plot wind direction
    title_label='Wind direction - Averaged vertical profiles'
    plt.figure(figsize=(1676./my_dpi, 1303./my_dpi), dpi=my_dpi) 
    plt.rcParams['axes.labelsize'] = font_size
    plt.rcParams['xtick.labelsize'] = font_size
    plt.rcParams['ytick.labelsize'] = font_size
    plt.plot(data.wd[:,0],data.heights,color='r',linewidth=1,label=stringheora_ut[0]+' UT - '+stringheora_ut[0]+' MST')
    plt.plot(data.wd[:,1],data.heights,color='b',linewidth=1,label=stringheora_ut[1]+' UT - '+stringheora_ut[1]+' MST')
    plt.plot(data.wd[:,2],data.heights,color='g',linewidth=1,label=stringheora_ut[2]+' UT - '+stringheora_ut[2]+' MST')
    plt.legend(loc='lower left',bbox_to_anchor=(0., 1.1), shadow=True, fontsize=font_size)
#        plt.axes().set_aspect(0.36)
    plt.xticks(angle_values,angle_names)
    plt.tick_params(which='both')
    ...
    plt.tight_layout()
#    plt.subplots_adjust(left=0.09, right=0.95, top=0.77, bottom=0.12) 
    fileout="DATE_winddir_pv_avg_ALL.png"
    plt.savefig(fileout,dpi=my_dpi)
    print("Creato file con plot:", fileout)
...
if __name__ == '__main__':
    try:
        ...
        print("!!!Error: error plotting plot_wind_pv_avg.py with input parameters: ",sys.argv)
