...
class Datastore(object):
    def __init__(self,files) :
        ...
        #Get data in Celsius
        #We do this because of axis arangements (left Celsius, right Kelvin)
        self.temp=data_all[:,1,:]-273.15
        self.temp_sd=data_all[:,2,:]
...
def main():
    ...
    #switch back to kelvin
    printtime=data.time[start_index:end_index+1]
    printtemp=data.temp[start_index:end_index+1,:]+273.15
    printtemp_sd=data.temp_sd[start_index:end_index+1,:]
    ...
    for i in range(nfiles):
        ...
        #Set limits
        maxval=np.max(data.temp[start_index:end_index]+data.temp_sd[start_index:end_index])
        minval=np.min(data.temp[start_index:end_index]-data.temp_sd[start_index:end_index])
        if maxval<0:
            maxval=maxval-maxval/30.
        else:
            maxval=maxval+maxval/30.
        if (minval<0):
            minval=minval+minval/30.
        else:
            minval=minval-minval/30.

        #Plot temp
        title_label='Temperature - Temporal evolution on level k='+level.strip()
        ...
        plt.errorbar(data.time,data.temp[:,i], yerr=data.temp_sd[:,i],color='k',linewidth=2, ecolor='#878b91',elinewidth=0.5,capsize=2)
...
#        plt.axes().set_aspect(0.36)
        ...
        plt.ylabel('Temperature ($^o$C)',fontsize=font_size+2)
        ax = plt.gca()
        ...
        ax1Ticks_Y = ax1.get_yticks()
#        starttick=(0.1*round((np.min(ax1Ticks_Y)+273.15)/0.1))-273.15
#        ax1Ticks_Y=np.arange(starttick,maxval+maxval/50.,round((maxval-minval)/6,1),dtype=float)
        ax3.set_yticks(ax1Ticks_Y)
        ax3Ticks=ax1Ticks_Y+273.15
        for idxt in range(len(ax3Ticks)):
            ax3Ticks[idxt]=round(ax3Ticks[idxt],1)
        ax3.set_ybound(ax1.get_ybound())
        ax3.set_yticklabels(ax3Ticks,fontsize=font_size+1)
        ax3.set_ylabel('(K)',fontsize=font_size+1)
        ...
        plt.tight_layout()
#        plt.subplots_adjust(left=0.13, right=0.86, top=0.84, bottom=0.11) 
 
        fileout="DATE_tempabs_k"+level.strip()+"_evol_time.png"
        plt.savefig(fileout,dpi=my_dpi)
        print("Creato file con plot:", fileout)

        #Write out dat file
        fname="DATE_tempabs_k"+level.strip()+"_evol_time.dat"
        f = open(fname, 'w')
        f.write('### TEMPERATURE LEVEL K='+level.strip()+' TIME EVOLUTION'+'\n')
        f.write('### TIME(h) TEMP(K) SIGMA_TEMP(K)'+'\n')
        printdata=np.zeros((1,3),float)
        for j in range(len(printtime)):
            printdata[0,0]=printtime[j]
            printdata[0,1]=printtemp[j,i]
            printdata[0,2]=printtemp_sd[j,i]
...
if __name__ == '__main__':
    try:
        ...
        print("!!!Error: error plotting plot_temp_level_time.py with input parameters: ",sys.argv)
